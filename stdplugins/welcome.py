...
async def _(event):
    ...
    if cws:
        ...
        if event.user_joined:
            ...
            cname = "{}".format(chat.title)
            count = "{}".format((await borg.get_participants(event.chat_id)).total)
            time = "{}".format(datetime.now().strftime("%H:%M"))
            current_message = await event.reply(
                current_saved_welcome_message.format(mention=mention, count=count, cname=cname, time=time)
...
async def _(event):
    ...
    if msg and msg.media:
        ...
        await event.edit("```Welcome media saved.```")
    else:
...
@borg.on(admin_cmd("welcome"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
